...
def setup_config(config_fpath: str, args: argparse.Namespace,weights_path):
    cfg = get_cfg()
...
class DenseNet(nn.Module):
    def __init__(self,weights_path, indexes=None):
        super().__init__()
        args = torch.load(os.path.join(thisdir, 'args_dict.pt'))
        args = argparse.Namespace(**args)
        cfg = setup_config(os.path.join(thisdir, args.cfg), args,weights_path=weights_path)
